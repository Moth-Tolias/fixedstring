...
struct FixedString(size_t maxSize)
{
	...
	/// constructor
    this(in string s) @safe @nogc nothrow pure
    {
        length = s.length;
        foreach (i, char c; s)
        {
	...
	/// assignment
    public void opAssign(in string rhs) @safe @nogc nothrow pure
	...
	/// ditto
    public void opOpAssign(string op)(in string rhs) @safe @nogc nothrow pure if (op == "~")
    in (rhs.length <= maxSize)
    {
	...
	/// ditto
    public bool opEquals(in string s) @safe @nogc nothrow const pure
    {
        if (length != s.length)
        {
            return false;
